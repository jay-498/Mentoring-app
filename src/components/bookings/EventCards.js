    ...
    {events.length ? events.map((event)=>(
        <div key={event._id} className="flex-col p-5 mt-5 w-full border rounded-lg">
            <div className="flex justify-between items-center">
                <p className="font-Manrope font-medium text-[#888585] text-[14px]">{event.mentor.description}</p>
            ...
            <div className="flex items-center gap-x-3 pt-2 pb-4">
                <img src={calender} alt="c"/>
                <p className="font-Manrope font-medium text-[#bbb9b9] text-[12px]">Fees - {event.mentor.session_rate} INR</p>
            </div>
