...
export class calender extends Component {
    ...
    tileDisabled = ({ date, view }) => {
        if (view === 'month') {
            if (this.props.unbooked_dates.find(dDate => this.isSameDay(new Date(dDate), date))) {
                return true;
            }
        }
    }
    ...
    render() {
        console.log(this.props.unbooked_dates);
        return (
            <div className="flex items-center p-2 pt-0">
                <div className="max-w-3xl shadow-xl pb-3">
                    ...
                    <div className="p-6 flex">
                        <Calendar tileClassName={this.tileClassName} tileDisabled={this.tileDisabled} className="p-5 h-full" onChange={this.onChangeDate} value={this.state.date} />
                        <div>
                            ...
                            <div className="pt-0 h-96 grid gap-4 grid-cols-1 grid-rows p-10 overflow-y-auto scrollbar-hide">
                                {this.props.timings.map((time) => (
                                    <button key={time} onClick={() => this.props.updateBookingTime(time)} className="bg-transparent hover:bg-cyan-500 text-cyan-700 font-semibold hover:text-white py-2 px-4 border border-cyan-500 hover:border-transparent rounded">
                                        {time}
                                    </button>
                                ))}
                            </div>
    ...
    return {
        updateBookingDate: data => {
            if (data > new Date()) {
                ...
                alert("Please Kindly Book Your Session From Tommorrow")
        },
        updateBookingTime: data => {
            if (isDateSet)
                dispatch(updateBookingTime(data));
            else
                alert("Please Kindly Select Your Date")
        }
    };
};
